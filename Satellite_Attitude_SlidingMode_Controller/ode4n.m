function Y = ode4n(odefun,tspan,y0,cmode,varargin)
% ODE4N modified ODE4 fixed step size integrator
% AE 6356 Assignment 6 Problem 6
...
%% sliding mode control law (switching)
if cmode==1   
  ...
  %
    ...
    if (ti/sample_rate-floor(ti/sample_rate))<0.001 % test to see if this is a control update (1 msec test threshold)
      if (ti~=tlast)    % execute if this is not the first sample
        ...
        s=(w-wcs)+kp*sign(dq4)*dq3;
        L=J*(kp/2*(norm(dq4)*(wcs-w)-sign(dq4)*cross(dq3,(wcs+w)))+wcds-ks*sign(s))+cross(w,J*w);
...
%
%% sliding mode control law (saturation)
if cmode==2  
  % initialize parameters 
  tlast=0;
  kp=1; %gain     
  ...
  %
    ...
    if (ti/sample_rate-floor(ti/sample_rate))<0.001 % test to see if this is a control update (1 msec test threshold)
      if (ti~=tlast)    % execute if this is not the first sample
        ...
        s=(w-wcs)+kp*sign(dq4)*dq3;
        L=J*(kp/2*(norm(dq4)*(wcs-w)-sign(dq4)*cross(dq3,(wcs+w)))+wcds-ks*sat(s))+cross(w,J*w);
